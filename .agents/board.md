...
- **Siguiente:** Prompt 1 GHCR OK — workflow run `26926489943`, tag `core-first-24e070d7`. Prompt 2 bloqueado: ACA pull `DENIED` al actualizar `aeco-pdf-parser`; rollback aplicado a `aeco-source-crawler`; tracking issue #454. Siguiente: actualizar ACA secret `ghcr-pat` o hacer packages pullables por ACA, luego reintentar update 3 jobs + D6.1e run/verify Windows/Azure-auth · Friday retro
