...
- El test general post-acciones tambien quedo cerrado: se detecto y corrigio un `uvicorn worker.app` huerfano que dejaba `umbral-worker.service` en auto-restart, se endurecio la carga de `~/.config/openclaw/env` para valores con espacios y se valido de nuevo el wiring completo con OpenClaw, Worker, Linear, Calendar, `research.web`, cron y paneles. Ver `docs/audits/openclaw-post-actions-validation-2026-03-24.md`.
