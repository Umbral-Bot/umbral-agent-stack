# Granola Night Watch — 2026-03-31 run4

Generated: 2026-03-31 22:55:00 -03:00

## Summary

- Worker local `http://127.0.0.1:8088/health` OK; 86 tasks registered.
- Granola handlers present: `granola.capitalize_raw`, `granola.create_human_task_from_curated_session`, `granola.promote_operational_slice`, `granola.update_commercial_project_from_curated_session`, `granola.promote_session_capitalizable`, `granola.promote_curated_session`, `granola.process_transcript`, `granola.create_followup`.
- Live raw backlog is now 50 rows in `Transcripciones Granola` (`3265f443-fb5c-81d7-89b9-e16eacb0082d`): 6 smoke/test rows, 3 promoted real raws, and 41 pending/non-promoted rows.
- No live worker failures surfaced in authenticated `/tasks`; recent task statuses were all `done`.
- Repo-accessible worker logs in [`C:/GitHub/umbral-agent-stack-codex/.tmp/worker-local-8088.err.log`](C:/GitHub/umbral-agent-stack-codex/.tmp/worker-local-8088.err.log) and [`C:/GitHub/umbral-agent-stack-codex/.tmp/worker-local-8088.out.log`](C:/GitHub/umbral-agent-stack-codex/.tmp/worker-local-8088.out.log) show startup, pings, and no Notion validation/runtime errors.
- Latest VM raw intake report [`C:/Granola/reports/granola-vm-raw-intake-latest.json`](C:/Granola/reports/granola-vm-raw-intake-latest.json) is healthy at `2026-03-31T23:15:54Z` with `selected_count=0`, `prepared_count=0`, and worker preflight OK.
- No safe idempotent fix was applied in this run.

## Poller

- Direct live VPS PID/log verification remains unavailable from this sandbox.
- Singleton/stale-PID guard is still present in [`C:/GitHub/umbral-agent-stack-codex/scripts/vps/notion-poller-daemon.py`](C:/GitHub/umbral-agent-stack-codex/scripts/vps/notion-poller-daemon.py):
  - `_claim_pid_file()` refuses startup if another PID is alive.
  - stale PID entries are removed before startup.
  - `SIGTERM`/`SIGINT` handlers keep cleanup graceful.
- `python -m py_compile scripts/vps/notion-poller-daemon.py` passed.

## Changed Smoke

These remain excluded from the real backlog, but one known 2026-03-17 smoke row changed status and therefore must be reported:

- `3265f443-fb5c-81f7-9c31-ee53d2126796` | `Smoke Granola memoria operativa 2026-03-17` | <https://www.notion.so/Smoke-Granola-memoria-operativa-2026-03-17-3265f443fb5c81f79c31ee53d2126796>
  - current status: `Procesada`
  - traceability: `Proyecto 3205f443-fb5c-81e2-b58e-ee73c01a0804 | Entregable 3265f443-fb5c-81fb-98c6-e08a3801e206 | task_id granola-capitalization-smoke-2026-03-17`
  - fields: `Fecha que Rick pasó a Notion=2026-03-17`, `Fecha que el agente procesó=` blank, `URL artefacto=` blank
  - action: reported only; no fix applied because it is still a smoke artifact, not a real meeting.

## Promoted Real Raws

These 3 real raws remain promoted and keep raw governance fields synced:
...
- `3305f443-fb5c-81db-9162-fd70c8574938` | `Konstruedu` | <https://www.notion.so/Konstruedu-3305f443fb5c81db9162fd70c8574938>
  - route executed: `granola.promote_curated_session`
  - route missing: `granola.promote_operational_slice`
  - blocker: base promotion OK; still carries source-level duplicate review blocker from Granola document `1d177374-2ff0-42a0-a032-189075f8b4c0`
  - Rick acknowledged: yes
...
- `3305f443-fb5c-81e6-a1a5-cc0b2ebd1786` | `Asesoría discurso` | <https://www.notion.so/Asesor-a-discurso-3305f443fb5c81e6a1a5cc0b2ebd1786>
  - route executed: `granola.promote_curated_session`
  - route missing: `granola.promote_operational_slice`
  - blocker: base promotion OK; still carries source-level duplicate review blocker from Granola document `5bff2a5a-0c7a-41c6-ae34-d6662325d67f`
  - Rick acknowledged: yes
  - curated page: `3305f443-fb5c-81f5-9911-c9be3fab3c42` | <https://www.notion.so/Asesor-a-discurso-3305f443fb5c81f59911c9be3fab3c42>
  - governance fields: `Estado=Procesada`, `Fecha que Rick pasó a Notion=2026-03-31`, `Fecha que el agente procesó=2026-03-31`, `URL artefacto` synced

## Pending / Partial Promotion

Common live state for the 41 non-promoted rows below:
...
- blocker: no human review and no explicit promotion payload yet
- Rick acknowledged: no (`comments_count=0` on every row)

### Partially promoted / drifted rows
...
- `3345f443-fb5c-817f-a2a3-fdbc9285a5e1` | `Revisión final de proyectos de módulo de inteligencia artificial generativa` | <https://www.notion.so/Revisi-n-final-de-proyectos-de-m-dulo-de-inteligencia-artificial-generativa-3345f443fb5c817fa2a3fdbc9285a5e1>
  - current raw status: `Procesada`
  - artifact page only: <https://www.notion.so/Revisi-n-final-de-proyectos-de-m-dulo-de-inteligencia-artificial-generativa-733fe941bfbf44358535b87f9ec8329e>
  - missing governance fields: `Fecha que Rick pasó a Notion`, `Fecha que el agente procesó`
  - blocker: artifact exists but there is no curated DB linkage, no comments, and no verified promotion route beyond raw ingest

- `3355f443-fb5c-812b-ba58-f853c452e744` | `[TMP VALIDATION] Granola ef88925a` | <https://www.notion.so/TMP-VALIDATION-Granola-ef88925a-3355f443fb5c812bba58f853c452e744>
  - current raw status: `Procesada`
  - artifact page only: <https://www.notion.so/TMP-VALIDATION-Granola-ef88925a-eb010e3976614df2ad5e26beef89d9df>
  - missing governance fields: `Fecha que Rick pasó a Notion`, `Fecha que el agente procesó`
  - blocker: validation artifact leaked into the real raw DB; no curated DB linkage, no comments, no verified promotion route beyond raw ingest

### Remaining pending real raws
...
- `3345f443-fb5c-8152-bd56-e1b3b336bc3d` | `asesoría discurso` | <https://www.notion.so/asesor-a-discorso-3345f443fb5c8152bd56e1b3b336bc3d>
