...
2. El clasificador de intención trataba cualquier `?` como pregunta, incluso cuando el carácter venía de mojibake dentro de palabras como `regularizaci?n`.
3. Las tareas creadas por `_handle_instruction()` usaban solo los primeros 8 caracteres del `comment_id`, lo que generaba colisiones en comentarios distintos con el mismo prefijo.
...
6. `_handle_instruction()` enviaba la segunda actualización con `status = in_progress`, pero la base `Tareas` solo acepta `queued`, `running`, `done`, `blocked` y `failed`; eso dejaba algunas tareas con resumen actualizado pero `Status = queued`.
...
- si el filtro de deliverables por `Estado revision` falla, el poller reintenta sin filtro
...
- `Benchmark parcial de Kris Wojslaw para el embudo`
  ...
  - conserva un solo comentario de revisión:
    - `trabajo incompleto`
  - esto es correcto: no necesitaba comentario nuevo
- tarea asociada:
  - `Task ID = notion-instruction-3265f443`
  - `Status = done`
  - `Proyecto` y `Entregable` ligados correctamente
  - conclusión: el frente Kris quedó semánticamente cerrado como benchmark parcial rechazado; el error estaba en la interpretación de la auditoría, no en la ausencia de comentario nuevo
...
### Corrección del residual de auditoría
- la tarea histórica projectless creada antes del hardening quedó:
  - `Status = blocked`
  - ligada a `Proyecto Embudo Ventas`
  - con resumen explícito de que fue superada por el follow-up posterior
- la nueva instrucción project-scoped quedó:
  - `Status = running`
  - ligada desde el nacimiento a `Proyecto Embudo Ventas`
  - con título humano y sin texto roto por encoding

## Resultado
El sistema ya no depende solo de `Control Room` para captar comentarios relevantes y deja de confundir mojibake con preguntas reales. Además, las instrucciones desde Notion ya no deberían pisarse entre sí cuando comparten prefijos similares de `comment_id`, y si nacen desde una página de proyecto o entregable quedan mejor contextualizadas.
...
- el filtro de deliverables en el worker remoto aún devuelve `500` en algunos casos; el poller ya hace fallback sin romper el flujo, pero la causa raíz del handler remoto sigue pendiente si se quiere dejar impecable
