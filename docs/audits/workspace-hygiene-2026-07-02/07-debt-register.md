...
| P0-2 | **Commit sin push + 58 dirty en clone base** | `a85563b8` (CAND-PROD-001 stage3) solo local; dirty editorial/OpenClaw overrides desde jun-06 | Cursor lead — ejecutar rescate Pass 8 (gate G-WH-1) |
...
| # | Deuda | Detalle | Owner sugerido |
|---|---|---|---|
| P1-1 | **16 dirty editoriales en coordinador** | contratos human-review + calibraciones skills no en main | Codex — rescate Pass 8 |
...
| P1-5 | **Ramas locales fósiles en `-copilot`** | 20+ con upstream gone/merged | Copilot Windows post-G-WH-1 |
...
| P1-9 | **`scripts/ops/*.ps1` huérfanos untracked** | `evaluate-gate.ps1`, `p10-sec63-*.ps1` en `-copilot` sin dueño claro | Copilot Windows — adjudicar a hilo (graphify o PIT) o descartar |
