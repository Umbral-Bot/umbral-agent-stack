...
| **F8A.prompt-quoting-fix** | **⏳ PR pending** | **fixes second wrapper defect found by stdin retry: replace nested `$(cat \"$prompt_file\")` in `--prompt` with `prompt=$(cat "$prompt_file")` + `--prompt "$prompt"`. Retry task requires DNS/egress non-empty before opening L3/L4. D41.** |
