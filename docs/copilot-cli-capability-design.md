...
| D29 | F7 rehearsal 1: abrir **únicamente** el L2 policy gate (`copilot_cli.enabled: false → true` en `config/tool_policy.yaml`) sin tocar L3/L4/L5. Objetivo: observar en live que el rejection de `copilot_cli.run` transiciona de `policy_off` a `execute_flag_off_dry_run`, demostrando que las capas posteriores sostienen el bloqueo de manera independiente. **Diverge intencionalmente del orden canónico G3→G4→G1→G2 del playbook 6C-4F** porque no es activación: G2 (`RICK_COPILOT_CLI_EXECUTE`) sigue `false`, L5 (`_REAL_EXECUTION_IMPLEMENTED`) sigue `False`, L4 (`copilot_cli.egress.activated`) sigue `false`, ninguna ejecución real ocurre. Tests actualizados: `test_capability_disabled_when_policy_off` y `test_f4_master_switch_still_off` ahora son robustos al cambio de yaml; 4 tests nuevos `test_f7_rehearsal_*` pinean el contrato (yaml policy=true, egress=false, _REAL_EXECUTION_IMPLEMENTED=False, end-to-end env+policy on/execute off → `execute_flag_off_dry_run` con subprocess explosion guard). Doc nuevo: `docs/copilot-cli-f7-policy-gate-rehearsal.md` con expected-shape antes/después, deploy procedure, rollback (single-yaml-flip, no irreversible state). PR-only en este momento; deploy queda pendiente de aprobación humana en step posterior; live worker sigue en `73ae88b` PID `1124888` sin tocar | ✅ |
| D30 | F7 rehearsal 1 — live deploy evidence: PR #274 (`rick/copilot-cli-f7-policy-gate-rehearsal`) mergeado a `main` (merge commit `6ce6cae`). Deploy: `git pull --ff-only` en live worktree (HEAD `45ff7e1 → 6ce6cae`), 1 restart único (MainPID `1124888 → 1403101`), `/health=200`. Probe confirmó transición de señal: antes `capability_disabled/policy_off`, después `ok:true/would_run:false/decision:execute_flag_off_dry_run/policy_enabled:true/execute_enabled:false/egress_activated:false`. Gates cerrados verificados: `RICK_COPILOT_CLI_EXECUTE=false` (L3), `_REAL_EXECUTION_IMPLEMENTED=False` (L5), `egress.activated=false` (L4). Audit log gitignored + token-scan clean. Sin nft/Docker/Copilot HTTPS/Notion/publish. Evidence doc: `docs/copilot-cli-f7-policy-gate-rehearsal-live-evidence.md` | ✅ |
| D31 | F7 rehearsal 2 — execute gate live rehearsal: flip L3 únicamente (`RICK_COPILOT_CLI_EXECUTE=false → true` en envfile live `/home/rick/.config/openclaw/copilot-cli.env`), sin cambio de repo. Baseline probe: `execute_flag_off_dry_run` (PID `1403101`). Flip+restart (PID `1403101 → 1415230`): probe confirmó transición a `real_execution_not_implemented` — `execute_enabled=true`, `real_execution_implemented=false`, `would_run=false`, `egress_activated=false`. Sin Copilot HTTPS, sin subprocess, sin nft/Docker. Audit token scan clean. Rollback inmediato: restore bak.f7r2 + restart (PID `1415230 → 1415290`) → `RICK_COPILOT_CLI_EXECUTE=false` restaurado, `/health=200`. Los 5 gates (L1-L5) verificados individualmente en rehearsals 1+2: L5 `_REAL_EXECUTION_IMPLEMENTED=False` es el último bloqueo absoluto antes de ejecución real. Evidence doc: `docs/copilot-cli-f7-execute-gate-rehearsal-live-evidence.md` | ✅ |
| D32 | F7 rehearsal 3 — egress/sandbox readiness check (no real execution): verificados sin levantar ningún gate. (1) Env/token contract: `verify_copilot_cli_env_contract.py --strict` → `OK — no findings`; 4 vars por nombre en process env (no valores). (2) Sandbox scripts: todos presentes en `worker/sandbox/` (Dockerfile, copilot-cli-wrapper, copilot-cli-smoke, run-copilot-cli-smoke.sh, refresh-copilot-cli.sh, test-copilot-cli-wrapper.sh). (3) Docker: v29.2.1 accesible, Cgroup v2/systemd, rootless capable. (4) Sandbox image `umbral-sandbox-copilot-cli`: **NO BUILT** — build step pendiente de aprobación explícita; offline smoke SKIPPED. (5) Egress: `verify_copilot_egress_contract.py` → `OK`; resolver dry-run resolvió 5 endpoints allowlisted (no nft aplicado). (6) Live task probe: `decision=execute_flag_off_dry_run`, `would_run=false`, `phase_blocks_real_execution=true`, sin Copilot HTTPS. (7) Audit token scan: `reports/copilot-cli/2026-05/d32a206146e44b95b78cb69528572e0d.jsonl` → clean. MainPID `1415290` sin cambio (NO restart). Gates finales: L3=false, L4=false, L5=False. Evidence doc: `docs/copilot-cli-f7-readiness-rehearsal-live-evidence.md` | ✅ |
| D33 | F7 rehearsal 4A — sandbox image build + offline smoke (no real execution): imagen `umbral-sandbox-copilot-cli:6940cf0f274d` construida desde `worker/sandbox/Dockerfile.copilot-cli` (tag determinístico = sha256[:12] de Dockerfile+smoke+wrapper). Build usó red solo para `node:22.14-bookworm-slim` + `npm install @github/copilot@1.0.36` — NO Copilot HTTPS runtime, NO créditos consumidos, NO token pasado al build. Image ID `sha256:fe0daf3dc717`, 223 MB, `@github/copilot` v1.0.36 instalado en sandbox. Host `which copilot` sigue ausente. **Smoke 11/11 PASS**: uid=10001 non-root, seccomp=2/no-new-privs/CapEff=0 (cap-drop=ALL), copilot binary v1.0.36 presente, no auth env leaked, DNS blocked (--network=none), /work read-only, /scratch writable (tmpfs), wrapper blocks git-push + rm-rf. **Wrapper tests 60/60 PASS**: 53 banned cases (git push variants, gh pr/release/repo/secret/auth/api/workflow, rm-rf/chmod-R/chown-R/dd/mkfs/sudo/su) + 7 allowed cases. MainPID `1418206` sin cambio (NO restart). Task probe: `execute_flag_off_dry_run`, `would_run=false` (L3/L4/L5 intactos). Sin nft/Docker network. Evidence doc: `docs/copilot-cli-f7-sandbox-image-live-evidence.md` | ✅ |
| D35 | F7 rehearsal 4C — nft live apply + rollback (no real execution): backup ruleset 544 líneas a `~/.copilot/backups/nft-ruleset-before-f7r4c-20260505T181309Z.nft` (chmod 0600). Parse-check `sudo nft -c` exit=0. **Apply live** `sudo nft -f /tmp/copilot-egress-f7r4c.nft` exit=0 — `table inet copilot_egress` creada con 3 chains (output/input/forward) + 2 sets (copilot_v4/copilot_v6 vacíos). Worker PID `1418206` unchanged, `/health=200` durante tabla activa. Task probe con tabla activa: `execute_flag_off_dry_run`, `would_run=false`, `egress_activated=false` (L3+L4 policy siguen cerrados). **Rollback inmediato**: `sudo nft delete table inet copilot_egress` exit=0 → tabla removida. Post-rollback: PID `1418206`, `/health=200`. Todos los gates finales sin cambio. nft mechanics validados end-to-end: apply funciona, rollback limpio, worker no afectado por presencia/ausencia de tabla. IP sets vacíos en este rehearsal — resolver debe poblarlos antes de activación real. Evidence doc: `docs/copilot-cli-f7-nft-live-rollback-evidence.md` | ✅ |
| D34 | F7 rehearsal 4B — egress profile live-staging, parse-check only (no real execution): egress artifacts inventariados (nft.example 3477B, resolver 10820B, verifier 10485B). Contract verifier `OK`. Resolver dry-run: 6 endpoints → 3 IPv4 addrs (`140.82.113.21` × 4 Copilot hostnames, `4.228.31.149` api.github.com, `4.228.31.153` copilot-proxy.githubusercontent.com), no IPv6, `would_apply=false`. Template copied to `/tmp/copilot-egress-f7r4b.nft` (93 lines). **`sudo nft -c -f` exit=0** (syntax VALID); non-root `nft -c` fails with netlink permission error (expected — kernel cache init requires root even for check-only). **No `nft -f` run**. No live nft rules applied. No Docker network created. MainPID `1418206` unchanged (NO restart). Task probe: `execute_flag_off_dry_run`, `would_run=false`, `egress_activated=false`. All 5 gates unchanged. Egress profile: artifacts ready, template syntax valid, resolver resolves all 6 allowlisted endpoints; awaiting operator approval + `_REAL_EXECUTION_IMPLEMENTED=True` PR before live apply. Evidence doc: `docs/copilot-cli-f7-egress-staging-live-evidence.md` | ✅ |
| D36 | F7 rehearsal 5A — code gate open (no real execution): `_REAL_EXECUTION_IMPLEMENTED = False -> True` in `worker/tasks/copilot_cli.py`. L3 (`RICK_COPILOT_CLI_EXECUTE=false`) remains the operator kill-switch and is checked before L5, so expected post-deploy signal is still `execute_flag_off_dry_run` with `real_execution_implemented=true`. Tests updated to prove L5 true + L3 false blocks, and L5 true + L3 true + `dry_run=True` does not launch subprocess. No nft, no Docker network, no Copilot HTTPS, no runtime deploy in this PR. Evidence/design doc: `docs/copilot-cli-f7-code-gate-rehearsal.md` | ✅ |
| D37 | F8A — real execution path implemented behind L1-L5 (PR-only): `copilot_cli.run` now calls `subprocess.run(docker run ...)` only when L1 env, L2 policy, L3 execute, L4 egress, L5 code gate are all open and `dry_run=false`. L3 false still returns `execute_flag_off_dry_run`; L4 false with `dry_run=false` returns `egress_not_activated`; `dry_run=true` never calls subprocess. Prompt is passed through stdin, token is passed by env name only (`--env COPILOT_GITHUB_TOKEN`), stdout/stderr are redacted before artifact write, manifest records `batch_id`/`agent_id`/`mission_run_id`/exit/duration/hashes/token+cost sources. Tests add mocked real-run coverage: no subprocess when gated, subprocess once when all gates open, token not in argv, prompt not in argv, artifacts/manifest written, token-shaped output redacted and marks run non-green. New docs: `docs/copilot-cli-f8a-real-execution-path.md`; task for Copilot-VPS: `.agents/tasks/f8a-first-real-run-2026-05-05.md`. No runtime activation by Codex. | ✅ |
| D38 | F8A bugfix after first real-run evidence: Copilot-VPS reached the actual sandboxed Copilot CLI binary (`mission_run_id=49a1496515c84826b215ae9d8ec400e9`) and failed with `exit_code=1` because the wrapper used obsolete CLI flag `--no-banner` (`error: unknown option '--no-banner'`). Infra path was validated and rollback restored L3=false/L4=false/nft removed. This PR removes only `--no-banner` from `worker/tasks/copilot_cli.py`; no gate flip, no runtime activation by Codex. | ✅ |
| D39 | F8A diagnostic mode after silent exit 1: retry `ee5aa7b921d44fdcb435d1c0803656d2` confirmed the `--no-banner` bugfix but Copilot CLI still exited `1` with stdout/stderr empty. Add `COPILOT_CLI_DIAGNOSTIC_MODE=true` support: real-run argv drops `--output-format=json --stream=off` and adds `--log-level=debug`, so Copilot-VPS can capture diagnostics without changing normal F8A behavior. New task: `.agents/tasks/f8a-diagnose-silent-exit-2026-05-06.md`. No gate flip, no runtime activation by Codex. | ✅ |
| D40 | F8A stdin fix after diagnostic evidence: Copilot-VPS diagnostic run `5d270087a2944283957bc3d21e4059e7` proved the prompt never reached the container because `docker run` lacked `-i`; wrapper `cat > $prompt_file` read EOF and Copilot was invoked with `--prompt ""`. Add `-i` to docker argv so `subprocess.run(input=prompt)` reaches container stdin. Diagnostic mode also stops using `--log-dir=/scratch/copilot-logs` so stderr/debug output is not lost in tmpfs. New retry task: `.agents/tasks/f8a-retry-after-docker-stdin-fix-2026-05-06.md`. No gate flip, no runtime activation by Codex. | ✅ |
| D41 | F8A prompt quoting fix after stdin evidence: retry `863af7a4e81c452faec1c06f94cf2132` confirmed PR #302 (`docker run -i`) works because the prompt file is populated inside the sandbox, but exposed a second wrapper defect: nested `--prompt "$(cat \"$prompt_file\")"` quoting under `sh -lc` resolves to an empty prompt. Fix by reading `prompt=$(cat "$prompt_file")` first and passing `--prompt "$prompt"`, removing the nested command substitution from the Copilot argv. New retry task: `.agents/tasks/f8a-retry-after-prompt-quoting-fix-2026-05-06.md`; it requires `APPROVE_F8A_PROMPT_QUOTING_RETRY=YES` and must stop before opening L3/L4 if the egress resolver returns empty IP sets (DNS issue found in the stdin retry). No gate flip, no runtime activation by Codex. | ✅ |
| D42 | F8A egress scope redesign after interrupted prompt-quoting retry: evidence PR #305 showed the nft template's host-wide `output` hook with `policy drop` blocked unrelated worker traffic (Notion poller) during an open run window. Redesign `infra/networking/copilot-egress.nft.example` to scope enforcement to dedicated Docker bridge `copilot-egress` / `br-copilot`: nft hooks only `forward`, keeps chain `policy accept`, accepts non-`br-copilot` traffic immediately, and drops only packets originating from the Copilot bridge that are not DNS or HTTPS to resolver-populated Copilot IP sets. Worker default `COPILOT_CLI_DOCKER_NETWORK` moves from `bridge` to `copilot-egress` so an unconfigured real run fails closed instead of bypassing scoped egress. Verifier now rejects host-wide output-drop artifacts. Staging evidence returned verde (host Notion/GitHub unaffected, container smoke through `copilot-egress` OK). No runtime activation by Codex. | ✅ |
| D43 | F8A clean first real-run retry: task `.agents/tasks/f8a-clean-first-real-run-retry-2026-05-07.md` executed by Copilot-VPS with approval. Run-5 (`mission_run_id=8dad30fe39f94c3781ee481d4d7d4349`) validated scoped egress, Docker network, stdin, prompt quoting, token env-by-name, nft drop counter 0, and rollback clean; Copilot still exited 1 with 0-byte stdout/stderr. Post-run diagnostics identified two blockers: missing writable `/home/runner/.copilot` tmpfs in read-only sandbox and expired/invalid `COPILOT_GITHUB_TOKEN` (GitHub API HTTP 401). Evidence PR #311 and code fix PR #312 merged. | ✅ |
| D44 | F8A writable Copilot home fix: add `--tmpfs /home/runner/.copilot:size=32m,mode=1777` to `worker/tasks/copilot_cli.py` Docker argv. This removes the silent startup failure where Copilot could not initialize `$HOME/.copilot` under `--read-only`. Local/VPS focused tests passed for `tests/test_copilot_cli.py`; remaining blocker is human token refresh. | ✅ |
| D45 | F8A run-6 after token refresh: task `.agents/tasks/f8a-run6-after-token-refresh-2026-05-07.md` executed by Copilot-VPS after token rotation. O0 returned `GITHUB_USER_HTTP=200`/`TOKEN_STATUS=valid` without printing token. Exactly one real `copilot_cli.run` executed (`mission_run_id=4c00f4de3f474abdbae75252725d58a8`), with scoped egress, writable Copilot home, stdin and prompt quoting all validated; result remained `exit_code=1`, stdout/stderr 0 bytes, and nft drop counter advanced 36 packets/2160 bytes. Rollback clean (L3=false, L4=false, diagnostic absent, nft/network removed, `/health=200`). Evidence report: `reports/copilot-cli/f8a-run6-after-token-refresh-2026-05-07.md`. | ✅ |
| D46 | F8B model-power and egress diagnostic prep + evidence: add optional `model` input to `copilot_cli.run`, validate it against `copilot_cli.allowed_models`, and pass it safely to Copilot CLI as `--model <name>` with shell quoting. Copilot-VPS direct diagnostic requested `Claude Opus 4.7`; the CLI accepted the flag, token validation passed, but scoped nft dropped 35 packets to sibling GitHub LB `140.82.113.21` while the resolver snapshot allowed a different A-record. Plan B not armed: all dropped traffic mapped to GitHub-owned CIDR. Evidence: `reports/copilot-cli/f8b-diagnose-egress-and-model-power-2026-05-07.md`. | ✅ |
| D47 | F8C GitHub Meta CIDR resolver fix: keep the resolver dry-run/default behavior unchanged, add explicit `--include-github-meta` to merge public GitHub Meta CIDRs (`api`, `web`, `copilot_api`) into `copilot_v4`/`copilot_v6`, and create a VPS retry task that verifies `140.82.112.0/20` reaches the nft set before rerunning the direct diagnostic. This addresses GitHub load-balancer round-robin without broadening to host-wide egress or triggering Plan B. | ⏳ |
...
| **F7.rehearsal-1** | **✅ done** | **policy gate flip-only + live deploy: PR #274 merged → `main` `6ce6cae`. Deploy: pull `45ff7e1→6ce6cae`, restart PID `1124888→1403101`, `/health=200`. Probe: `policy_off → execute_flag_off_dry_run`. L3 `RICK_COPILOT_CLI_EXECUTE=false`, L4 `copilot_cli.egress.activated=false`, L5 `_REAL_EXECUTION_IMPLEMENTED=False` intactos. Audit log clean. D29+D30. Evidence: `docs/copilot-cli-f7-policy-gate-rehearsal-live-evidence.md`** |
| **F7.rehearsal-2** | **✅ done** | **execute flag rehearsal + rollback: flip L3 (`RICK_COPILOT_CLI_EXECUTE=false→true`) en envfile live, 2 restarts (flip PID `1403101→1415230`, rollback PID `1415230→1415290`). Probe: `execute_flag_off_dry_run → real_execution_not_implemented`. Rollback inmediato: `RICK_COPILOT_CLI_EXECUTE=false` restaurado. Sin Copilot HTTPS/subprocess/nft/Docker. Audit clean. D31. Evidence: `docs/copilot-cli-f7-execute-gate-rehearsal-live-evidence.md`** |
| **F7.rehearsal-3** | **✅ done** | **egress/sandbox readiness (no real execution): env contract OK, sandbox scripts presentes, Docker v29.2.1 accesible, image not built (pending build approval), egress verifier OK + resolver dry-run, nft/Docker network absent, task probe `execute_flag_off_dry_run` (L3 still closed), audit clean, MainPID `1415290` sin restart. D32. Evidence: `docs/copilot-cli-f7-readiness-rehearsal-live-evidence.md`** |
| **F7.rehearsal-4A** | **✅ done** | **sandbox image build + offline smoke (no real execution): `umbral-sandbox-copilot-cli:6940cf0f274d` built (tag sha256-det.), smoke 11/11 PASS (uid=10001, seccomp/no-new-privs/cap-drop=ALL, copilot@1.0.36, DNS blocked, /work RO, wrapper blocks git-push+rm-rf), wrapper tests 60/60 PASS (53 banned + 7 allowed), host copilot absent, no token/no nft/no Docker network/no restart. D33. Evidence: `docs/copilot-cli-f7-sandbox-image-live-evidence.md`** |
| **F7.rehearsal-4B** | **✅ done** | **egress profile live-staging + parse-check (no real execution): contract verifier OK, resolver dry-run 6 endpoints → 3 IPv4 (140.82.113.21/4.228.31.149/4.228.31.153), `sudo nft -c` exit=0 (syntax valid), non-root nft-c needs root (expected), no `nft -f`, no live rules/Docker network/restart. Task probe `execute_flag_off_dry_run`. D34. Evidence: `docs/copilot-cli-f7-egress-staging-live-evidence.md`** |
| **F7.rehearsal-4C** | **✅ done** | **nft live apply + rollback (no real execution): backup 544L, parse-check exit=0, `sudo nft -f` exit=0 (table inet copilot_egress: 3 chains, 2 sets vacíos), worker PID 1418206/health 200 durante tabla activa, task probe `execute_flag_off_dry_run` (L3/policy L4 unchanged), rollback `sudo nft delete table` exit=0, tabla removida limpiamente. nft mechanics validados end-to-end. D35. Evidence: `docs/copilot-cli-f7-nft-live-rollback-evidence.md`** |
| **F7.rehearsal-5A** | **✅ done** | **code gate open + live deploy verified by Copilot-VPS: `_REAL_EXECUTION_IMPLEMENTED=True`, L3 (`RICK_COPILOT_CLI_EXECUTE=false`) remained closed, probe returned `execute_flag_off_dry_run` with `real_execution_implemented=true`; evidence merged via PR #293. D36. Evidence: `reports/copilot-cli/f7-5a-code-gate-deploy-2026-05-05.md`** |
| **F8A.real-execution-path** | **✅ done** | **actual Docker subprocess path behind L1-L5 + `dry_run=false` merged; Copilot-VPS first one-shot executed and produced `--no-banner` failure evidence. D37.** |
| **F8A.bugfix-no-banner** | **✅ done** | **minimal wrapper fix after first real-run evidence: obsolete `--no-banner` removed; retry evidence shifted failure from explicit flag error to silent exit. D38.** |
| **F8A.diagnose-silent-exit** | **✅ done** | **diagnostic mode added and used by Copilot-VPS; root cause traced to Docker stdin not being forwarded. D39.** |
| **F8A.stdin-fix** | **✅ done** | **docker `-i` merged so prompt stdin reaches the container; follow-up evidence exposed second wrapper quoting defect and DNS/egress empty-set blocker. D40.** |
| **F8A.prompt-quoting-fix** | **✅ done** | **fixes second wrapper defect found by stdin retry: replace nested `$(cat \"$prompt_file\")` in `--prompt` with `prompt=$(cat "$prompt_file")` + `--prompt "$prompt"`. Retry task attempted; no Copilot run fired because session was interrupted, but evidence found host-wide nft egress blocked Notion. D41.** |
| **F8A.egress-scope-redesign** | **✅ done** | **replaces host-wide nft `output policy drop` with scoped `forward policy accept` filtering on dedicated Docker bridge `br-copilot`; worker default network becomes `copilot-egress`; verifier rejects host-wide output drops; staging evidence verde confirmed host egress non-regression and container smoke. D42.** |
| **F8A.clean-real-run-retry** | **✅ done** | **run-5 executed exactly one real `copilot_cli.run` on scoped egress and rolled back cleanly; root causes found: missing writable `/home/runner/.copilot` tmpfs plus invalid/expired `COPILOT_GITHUB_TOKEN`; tmpfs fix merged. D43-D44.** |
| **F8A.run6-after-token-refresh** | **✅ done** | **token refresh succeeded; one real run executed and rolled back cleanly. Infra path clean, but Copilot CLI still exited 1 with empty output while scoped nft dropped 36 packets. D45.** |
| **F8B.egress-model-power-diagnostic** | **✅ done** | **direct diagnostic confirmed token valid and `--model "Claude Opus 4.7"` accepted by Copilot CLI, but nft dropped GitHub sibling LB `140.82.113.21` because the resolver snapshotted a different A-record. Plan B not armed; fix resolver with GitHub Meta CIDRs. D46.** |
| **F8C.github-meta-egress-resolver** | **⏳ next** | **add opt-in resolver flag `--include-github-meta` to merge GitHub Meta CIDRs (`api`, `web`, `copilot_api`) into nft IP sets, then rerun direct diagnostic with structured metrics and no canonical `copilot_cli.run`. D47.** |
| F9+ | ⏸ blocked | write-limited / PR-draft-limited / batch autónomo |
