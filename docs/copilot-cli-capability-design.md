...
| **F8A.real-execution-path** | **⏳ PR pending** | **implements actual Docker subprocess path behind L1-L5 + `dry_run=false`; no runtime activation by Codex. After merge, Copilot-VPS task `.agents/tasks/f8a-first-real-run-2026-05-05.md` performs deploy check and optional one-shot run only with David's explicit `APPROVE_F8A_ONE_SHOT_RUN=YES`. D37.** |
| **F8A.bugfix-no-banner** | **⏳ PR pending** | **minimal wrapper fix after first real-run evidence: remove obsolete `--no-banner`; retry task reuses F8A one-shot protocol after merge. D38.** |
| **F8A.diagnose-silent-exit** | **⏳ PR pending** | **adds diagnostic mode for silent Copilot CLI exit 1: `COPILOT_CLI_DIAGNOSTIC_MODE=true` removes JSON/stream flags and enables debug logging; Copilot-VPS task captures the next diagnostic run. D39.** |
| **F8A.stdin-fix** | **⏳ PR pending** | **adds docker `-i` so prompt stdin reaches the container; diagnostic mode drops `--log-dir` to expose stderr. D40.** |
