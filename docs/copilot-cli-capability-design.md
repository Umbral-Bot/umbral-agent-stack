...
| D36 | F7 rehearsal 5A — code gate open (no real execution): `_REAL_EXECUTION_IMPLEMENTED = False → True` en `worker/tasks/copilot_cli.py`. L3 (`RICK_COPILOT_CLI_EXECUTE=false`) sigue cerrado — bloquea antes que L5 en el decision tree. Tests actualizados (74/74): 3 assertions `is False → is True`, 1 test renombrado+reworked para L5=True+L3=True path (subprocess mocked), nuevo test `test_f7_5a_code_gate_open_but_execute_env_gate_blocks` (L5=True+L3=False → `execute_flag_off_dry_run`). Expected post-deploy: `decision=execute_flag_off_dry_run`, `real_execution_implemented=true`, `execute_enabled=false`. No nft, no Docker network, no Copilot HTTPS, no restart en esta fase. Evidence doc: `docs/copilot-cli-f7-code-gate-rehearsal.md` | ✅ |
...
| **F7.rehearsal-5A** | **⏳ deploy pending** | **code gate open: `_REAL_EXECUTION_IMPLEMENTED=True`. L3 (`RICK_COPILOT_CLI_EXECUTE=false`) sigue cerrado — expected post-deploy: `execute_flag_off_dry_run`, `real_execution_implemented=true`. 74/74 tests. No nft/Docker/Copilot HTTPS. D36. Evidence: `docs/copilot-cli-f7-code-gate-rehearsal.md`** |
