...
| **F7.rehearsal-1** | **🟡 PR open (awaiting merge+deploy)** | **policy gate flip-only: `config/tool_policy.yaml :: copilot_cli.enabled: false → true`. Único cambio funcional. L3 `RICK_COPILOT_CLI_EXECUTE=false`, L4 `copilot_cli.egress.activated=false`, L5 `_REAL_EXECUTION_IMPLEMENTED=False` mantenidos. Diverge del orden canónico G3→G4→G1→G2 intencionalmente (no es activación, es transición de signal de rejection). Tests: 73/73 verde local; 4 tests nuevos `test_f7_rehearsal_*` + 2 existentes hardened. Doc `docs/copilot-cli-f7-policy-gate-rehearsal.md` describe expected probe shape antes/después y rollback. D29 registrada. Live worker untouched (sigue `73ae88b` PID `1124888`); deploy es step separado post-merge** |
