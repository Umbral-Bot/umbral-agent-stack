...
| D42 | F8A egress scope redesign after interrupted prompt-quoting retry: evidence PR #305 showed the nft template's host-wide `output` hook with `policy drop` blocked unrelated worker traffic (Notion poller) during an open run window. Redesign `infra/networking/copilot-egress.nft.example` to scope enforcement to dedicated Docker bridge `copilot-egress` / `br-copilot`: nft hooks only `forward`, keeps chain `policy accept`, accepts non-`br-copilot` traffic immediately, and drops only packets originating from the Copilot bridge that are not DNS or HTTPS to resolver-populated Copilot IP sets. Worker default `COPILOT_CLI_DOCKER_NETWORK` moves from `bridge` to `copilot-egress` so an unconfigured real run fails closed instead of bypassing scoped egress. Verifier now rejects host-wide output-drop artifacts. New staging task: `.agents/tasks/f8a-redesign-egress-scope-2026-05-07.md`. No gate flip, no runtime activation by Codex. | ⏳ |
| D43 | F8A clean first real-run retry task: after scoped egress staging evidence `rick/f8a-egress-scope-staging-2026-05-07` returned verde (host Notion/GitHub unaffected, container smoke through `copilot-egress` OK), create a fresh runtime task `.agents/tasks/f8a-clean-first-real-run-retry-2026-05-07.md`. It requires `APPROVE_F8A_CLEAN_REAL_RUN_RETRY=YES`, opens L3/L4 only inside one run window, uses `COPILOT_CLI_DOCKER_NETWORK=copilot-egress`, captures manifest/audit/stdout/stderr/nft counters/journal evidence, and rolls back envfile/policy/nft/network/worker immediately. No runtime activation by Codex. | ⏳ |
...
| **F8A.egress-scope-redesign** | **⏳ PR pending** | **replaces host-wide nft `output policy drop` with scoped `forward policy accept` filtering on dedicated Docker bridge `br-copilot`; worker default network becomes `copilot-egress`; verifier rejects host-wide output drops. D42.** |
| **F8A.clean-real-run-retry** | **⏳ task pending** | **fresh single-run retry using scoped egress, diagnostic mode, one Copilot run only, and mandatory rollback. D43.** |
