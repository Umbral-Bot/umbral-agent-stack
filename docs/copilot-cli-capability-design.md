...
| D46 | F8B model-power and egress diagnostic prep: add optional `model` input to `copilot_cli.run`, validate it against `copilot_cli.allowed_models`, and pass it safely to Copilot CLI as `--model <name>` with shell quoting. The initial allowlist includes high-capability Copilot models listed in GitHub's supported-model docs, including `Claude Opus 4.7`, but actual availability is still enforced by GitHub plan/org model policy and the installed CLI. New task `.agents/tasks/f8b-diagnose-egress-and-model-power-2026-05-07.md` is a direct diagnostic follow-up: no second canonical `copilot_cli.run`; it must capture which scoped egress endpoints/IPs are dropped and verify model picker/`--model` behavior before widening tools. | ⏳ |
...
| **F8B.egress-model-power-diagnostic** | **⏳ next** | **enable safe `model` override for `copilot_cli.run`; prepare a direct diagnostic task to verify `--model` availability (target `Claude Opus 4.7`) and identify the missing Copilot egress endpoints before enabling broader tools. D46.** |
