...
| P1 (cron chmod) | **FIXED** (2026-08-08, PKG-UAS-VPS-CRON-CHMOD): `scripts/vps/dashboard-rick-cron.sh` + `openclaw-panel-cron.sh` marcados ejecutables en el índice git (`git update-index --chmod=+x`, `100644`→`100755`) y en el filesystem del VPS (`chmod +x`, desbloquea el cron ya mismo sin esperar merge). `bash -n` OK; no se corrió el cron completo (escribe a Notion, sin `--dry-run`). Stash `KEEP` original sin dropear hasta confirmar una corrida sana. — [uas-p1-vps-residuals-20260808.md](uas-p1-vps-residuals-20260808.md) | Claude VPS (sin self-merge) |
