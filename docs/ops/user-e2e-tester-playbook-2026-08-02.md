...
## 6. Suite P3 — contraste de fuentes (P3-01 y P3-02 ejecutadas 2026-08-04; re-corrida de P3-02 el 2026-08-05 → BLOCKED; re-corrida el 2026-08-06 → PASS de frescura, sin depender del fix #2)
...
| P3-02 | **Ejecutada 2026-08-04 — sub-sonda en PASS, código `B_PRIMARY_ONLY`** (`docs/ops/user-e2e-p3-02-freshness-2026-08-04.md`; el marcador de suite sigue abierto: faltan P3-03/P3-04). Mecánica validada: **David crea** el evento fresco en el **primary** —único calendario escribible— y a los ≥5 min el tester pregunta; el **alcance** se discrimina con eventos que **ya viven** en un calendario no-primary, sin crear nada ahí. Que lo cree el tester en vez de David es ampliación de superficie: exige orden escrita en el pack (así ocurrió el 2026-08-04, ver §2.4a de ese doc) | calendar UI | Frescura: el discriminador duro es pedir un dato que **sólo esté en el cuerpo** del evento y contrastarlo verbatim — título y horario no bastan para descartar reformulación de un briefing previo. Alcance: Rick listó sólo el primary. **`Umbral BIM` es una suscripción ICS de solo lectura** (Outlook/O365, id `@import.calendar.google.com`): no se puede crear ahí, y el fallo no es permiso mal configurado |
| P3-02 (re-corrida 2026-08-05) — *no confundir con el sufijo `b` de `P1b`, que designa una sub-sonda aparte con su propio GO y marcador* | **BLOCKED, no se midió** (`docs/ops/user-e2e-p3-02-calendar-2026-08-05.md`). Método que quedó validado: **preflight del canal ANTES de gastar el checkpoint de David** —enviar la pregunta con el oráculo ya leído— y, si falla, un mensaje de control **sin tools** (`hola`) para separar "Calendar caído" de "turno de agente caído" | chat + oráculo Calendar por API | Dos auth **independientes** pueden estar rotas a la vez: la de Calendar (briefing sin agenda) y la del **modelo en el gateway** (todo mensaje falla). El error accionable puede aparecer recién al 6.º intento: **no declarar BLOCKED con una sola lectura**, y no aplicar la tabla A–E si no hubo respuesta que clasificar |
| P3-02 (re-corrida 2026-08-06) | **PASS de frescura** (`docs/ops/user-e2e-p3-02-rerun-20260806.md`). El turno interactivo respondió normal (preflight PASS a la primera); el checkpoint del día no tenía ningún evento en `Umbral BIM` para probar alcance, así que la corrida solo mide frescura — con el discriminador más fuerte hasta ahora: una **pregunta genérica** de agenda sacó título+horario del evento fresco pero **no** el cuerpo (no basta); una **segunda pregunta dirigida** ("¿qué dice la descripción de X?") sí sacó el token verbatim. Método a incorporar: si la pregunta genérica no repite el cuerpo, preguntar por la descripción explícitamente antes de declarar FAIL de frescura | chat + oráculo Calendar UI | Frescura: PASS (token nunca escrito en el chat, citado exacto). Alcance: no discriminable ese día — sin evento en `Umbral BIM`, `B_PRIMARY_ONLY` no certifica punto ciego, solo ausencia de fabricación |
