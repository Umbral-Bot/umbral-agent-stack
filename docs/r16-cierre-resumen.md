...
| #89 | Recuperar scripts Bitácora | 🔄 Open | `scripts/enrich_bitacora_pages.py`, `scripts/add_resumen_amigable.py`, `tests/test_notion_enrich_bitacora.py` (desde `cursor/bit-cora-contenido-enriquecido`) |
| #90 | Recuperar rate limiter + dispatcher tests | 🔄 Open | `worker/rate_limit.py`, `tests/test_dispatcher_model_routing.py` (desde `feat/copilot-azure-foundry-audio`) |

## Estado final

- **Tests:** 536 passed (37 auth-related flaky)
...
1. **Mergear PRs #89 y #90** — contienen código de aplicación que requiere revisión
2. **Borrar ramas remotas** — ver `docs/guia-borrar-ramas-r16.md` (25 ramas candidatas)
3. **Enriquecer Bitácora** — una vez mergeado PR #89, ejecutar `scripts/enrich_bitacora_pages.py`
4. **Siguiente sprint (R17)** — planificar sobre la base consolidada
