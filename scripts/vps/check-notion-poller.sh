#!/usr/bin/env bash
# Check Notion Poller health â€” cron status, log tail, Redis state.
set -euo pipefail
...
if crontab -l 2>/dev/null | grep -qF "notion-poller-cron.sh"; then
    ...
    echo "   âŒ Cron NOT installed"
    echo "   â†’ Run: bash ~/umbral-agent-stack/scripts/vps/install-notion-poller-cron.sh"
fi
...
if [ -n "$LAST_TS" ]; then
    echo "   âœ… Last poll: $LAST_TS"
else
    echo "   âš ï¸  No timestamp found (poller has not run yet?)"
fi
...
if [ -f /tmp/notion_poller.log ]; then
    ...
    echo "   âš ï¸  Log file not found at /tmp/notion_poller.log"
fi
