...
def test_f4_master_switch_still_off():
    """F7 rehearsal 1: policy.enabled is now True in yaml, but execution
    gates must keep L3/L4 closed. L5 opened by F7 rehearsal 5A.
    """
    from worker import tool_policy as tp
    ...
    # L5 code constant: opened by F7 rehearsal 5A.
...
def test_f6_real_execution_implemented_constant_is_true():
    """F7 rehearsal 5A: code gate opened. L5 must now be True.
    Real execution is gated solely by RICK_COPILOT_CLI_EXECUTE (L3).
    """
    from worker.tasks import copilot_cli as mod
    assert mod._REAL_EXECUTION_IMPLEMENTED is True
...
def test_f6_execute_flag_on_still_blocked_by_real_execution_constant(monkeypatch):
    """F7 rehearsal 5A variant: with L5=True and L3=True, subprocess would
    be invoked. We mock subprocess to prevent actual execution and confirm
    the decision transitions to would_run_blocked_phase_f3.
    """
    _all_gates_open(monkeypatch, execute=True)
    import subprocess as _sp
    def _explode(*a, **kw):
        raise AssertionError("subprocess invoked — should not reach here with dry_run=True")
    ...
    # dry_run default=True, so decision is would_run_dry_run (not actual execution)
    assert res["decision"] in ("would_run_dry_run", "would_run_blocked_phase_f3")


def test_f7_5a_code_gate_open_but_execute_env_gate_blocks(monkeypatch):
    """F7 rehearsal 5A: L5=True, L3=False → execute_flag_off_dry_run.

    Verifies that opening the code gate alone is not sufficient to trigger
    execution — RICK_COPILOT_CLI_EXECUTE must also be true.
    """
    _all_gates_open(monkeypatch, execute=False)
    import subprocess as _sp
    def _explode(*a, **kw):
        raise AssertionError("subprocess invoked with execute gate closed")
    for name in ("run", "Popen", "call", "check_call", "check_output"):
        monkeypatch.setattr(_sp, name, _explode)
...
def test_f7_rehearsal_real_execution_constant_now_true():
    """F7 rehearsal 5A: code gate opened. L5 is now True; execution
    is still gated by L3 (RICK_COPILOT_CLI_EXECUTE).
    """
    from worker.tasks import copilot_cli as mod
    assert mod._REAL_EXECUTION_IMPLEMENTED is True
...
def test_f7_rehearsal_env_on_policy_on_execute_off_returns_dry_run(monkeypatch):
    ...
    the handler must:
      ...
      - keep policy.real_execution_implemented=true (L5 opened by F7.5A)
