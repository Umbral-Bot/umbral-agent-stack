...
def test_collect_candidate_comments_tolerates_session_capitalizable_query_failures():
    wc = MagicMock()
    wc.run.side_effect = [
    ...
    assert [call.kwargs for call in wc.notion_poll_comments.call_args_list] == expected_calls
    session_call_payload = wc.run.call_args_list[2].args[1]
    assert session_call_payload["database_id_or_url"] == "curated-db"
