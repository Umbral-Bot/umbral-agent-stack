...
def test_collect_candidate_comments_deduplicates_shared_page_ids_across_sources():
    wc = MagicMock()
    wc.run.side_effect = [
        {"ok": True, "result": {"items": [{"page_id": "shared-1"}]}},
        {"ok": True, "result": {"items": [{"page_id": "shared-1"}]}},
        {"ok": True, "result": {"items": [{"page_id": "shared-1"}]}},
    ]
    wc.notion_poll_comments.side_effect = [
                ...
                "comments": [
                    {
                        "id": "c-2",
                        "created_time": "2026-03-16T21:01:00.000Z",
                        "text": "comentario en pagina compartida",
                    }
                ]
            },
        },
    ...
    assert [comment["id"] for comment in comments] == ["c-1", "c-2"]
    assert comments[1]["page_id"] == "shared-1"
    assert comments[1]["page_kind"] == "deliverable"
    expected_calls = [
        {"since": "2026-03-16T20:55:00+00:00", "limit": 20, "page_id": None},
        {"since": "2026-03-16T20:55:00+00:00", "limit": 20, "page_id": "shared-1"},
    ]
    assert [call.kwargs for call in wc.notion_poll_comments.call_args_list] == expected_calls
