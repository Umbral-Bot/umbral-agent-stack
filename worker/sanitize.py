...
_TASK_FIELD_MAX_LEN = {
    ("granola.process_transcript", "input.content"): MAX_INPUT_JSON_BYTES,
    ("notion.write_transcript", "input.content"): MAX_INPUT_JSON_BYTES,
}
...
def _field_max_len(path: str, task: str | None) -> int:
    if task:
        return _TASK_FIELD_MAX_LEN.get((task, path), MAX_STRING_VALUE_LEN)
    return MAX_STRING_VALUE_LEN


def _sanitize_value(value: Any, path: str = "root", task: str | None = None) -> Any:
    """
    ...
    if isinstance(value, str):
        ...
        max_len = _field_max_len(path, task)
        if len(value) > max_len:
            logger.warning("Truncating field %r from %d to %d chars", path, len(value), max_len)
            return value[:max_len]
        return value
    if isinstance(value, dict):
        return {k: _sanitize_value(v, f"{path}.{k}", task) for k, v in value.items()}
    if isinstance(value, list):
        return [_sanitize_value(v, f"{path}[{i}]", task) for i, v in enumerate(value)]
    # primitives (int, float, bool, None) pass through
    return value


def sanitize_input(input_data: Dict[str, Any], task: str | None = None) -> Dict[str, Any]:
    """
    ...
    return _sanitize_value(input_data, "input", task)
